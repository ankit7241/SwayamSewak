...
export default function AddShgModal({ setOpenAddSHG }) {
    ...
    return (
        <div
            onClick={() => { navigate("/explore"); setOpenAddSHG(false) }}
        ...
        >
            ...
            >
                ...
                <h2 className='font-mammoth text-3xl tracking-wider text-white'>Create your own SHG</h2>
                        ...
                        {connected
                            ? <>
                                <p className='flex flex-row items-start px-20 py-10 bg-white/5 border-primaryWidth border-white/10 rounded-20 outline-none w-full font-medium text-sm text-white/70 text-center cursor-default'>
                                    {"Your address - "} <span className='text-white'>({address.slice(0, 5) + '...' + address.slice(-5)})</span>
                                </p>
                                <Button
                                    varient="light"
                                    gradient={false}
